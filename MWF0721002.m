function v=MWF0721002
v=zeros(110,2000);%pre-allocate memory for Matlab
...
a=0.02; %a is the cutting criteria for dP/dy transformation
...
t=[1 1081];
  f=[DPDY(2,65) 0];
  tt=linspace(1,1081,1081);
  DPDYINT=interp1(t,f,tt,'pchip');

for j=66:ymax+1 % 1079 rows
    if j/12==round(j/12), xstartpoint=xstartpoint-1; end
    for i=2:xstartpoint-1
    DPDY(i,j)=DPDYINT(j-64);
    end
end
