...
Vy0=2;%Vy0 is the initial velocity, which unit is m/s
...
t=[1 1238];
  f=[DPDY(2,1050) 0];
  tt=linspace(1,1238,1238);
  DPDYINT=interp1(t,f,tt,'pchip');

for j=1051:2286 % 1236 rows
    if j/12==round(j/12), xstartpoint=xstartpoint-1; end
    for i=2:xstartpoint-1
    DPDY(i,j)=DPDYINT(j-1049);
    end
end

for j=2287:ymax+1 % 2286 rows
    if j/12==round(j/12), xstartpoint=xstartpoint-1; end
    for i=2:xstartpoint-1
    DPDY(i,j)=0;
    end
...
for k=1:200000
  ...
  for j=2:ymax+1
    ...
    for i=2:xstartpoint-1
        ...
        if k>20000&&v(i,j)<0.00001, DPDY(i,j)=0;end
    end
  end
end